...
- **Silence detection**: SilenceDetector tracks consecutive silent audio callbacks. When silence exceeds a duration threshold (default 1.0s) and minimum chunk size (1s) is met, it returns the chunk index at silence onset + 0.5s buffer. The audio callback sets a flag; the tkinter level timer checks it and calls flush from the main thread.
